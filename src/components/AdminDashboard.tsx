...
function DashboardView({ totalRecaudado, totalEmitido, tasaRecaudacion, pagados, pendientes, vencidos, enMora }: any) {
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-gray-900">Resumen — Julio 2026</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-gray-200">
          ...
          <p className="text-xs text-gray-400 mt-1">de {formatGuaranies(totalEmitido)} emitido</p>
        </div>
        <div className="bg-white p-5 rounded-xl border border-gray-200">
          <p className="text-sm text-gray-500">Tasa de recaudación</p>
          <p className="text-2xl font-bold text-blue-600">{tasaRecaudacion}%</p>
          <p className="text-xs text-gray-400 mt-1">meta: 95%</p>
        </div>
        <div className="bg-white p-5 rounded-xl border border-gray-200">
          <p className="text-sm text-gray-500">Cobros del mes</p>
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-200">
          <p className="text-sm text-gray-500">Reclamos abiertos</p>
          <p className="text-2xl font-bold text-orange-600">2</p>
          <p className="text-xs text-gray-400 mt-1">1 en proceso, 1 nuevo</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="font-medium text-gray-900 mb-3">Últimos pagos recibidos</h3>
        <div className="space-y-2">
          {cobros.filter(c => c.estado === "pagado").sort((a,b) => (b.fechaPago || "").localeCompare(a.fechaPago || "")).slice(0, 5).map(c => (
            <div key={c.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
              <div>
                <p className="text-sm font-medium text-gray-900">Depto {c.unidadNumero} — {c.residente}</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Cobros — Julio 2026</h2>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Unidad</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Residente</th>
          ...
          <tbody>
            {cobros.map(cobro => (
              <tr key={cobro.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-4 py-3 text-sm font-medium text-gray-900">{cobro.unidadNumero}</td>
                <td className="px-4 py-3 text-sm text-gray-700">{cobro.residente}</td>
                <td className="px-4 py-3 text-sm font-medium text-gray-900">{formatGuaranies(cobro.monto)}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{cobro.vencimiento}</td>
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${getEstadoColor(cobro.estado)}`}>
                    {cobro.estado.toUpperCase()}
                  </span>
                </td>
                <td className="px-4 py-3 text-sm text-gray-500">{cobro.canalPago || "—"}</td>
                <td className="px-4 py-3">
...
function UnidadesView() {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Unidades — Torre Ñandutí</h2>
        <div className="flex gap-2">
          <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Agregar unidad</button>
          <button className="px-4 py-2 border border-gray-300 text-sm rounded-lg hover:bg-gray-50">Importar CSV</button>
        </div>
      </div>
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Unidad</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Piso</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Tipo</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">m²</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Coef.</th>
          ...
          <tbody>
            {unidades.map(u => (
              <tr key={u.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-4 py-3 text-sm font-medium text-gray-900">{u.numero}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{u.piso}</td>
                <td className="px-4 py-3 text-sm text-gray-500 capitalize">{u.tipo}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{u.metrosCuadrados}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{u.coeficiente}%</td>
                <td className="px-4 py-3 text-sm text-gray-700">{u.propietario}</td>
                <td className="px-4 py-3 text-sm text-gray-700">{u.residente}</td>
                <td className="px-4 py-3">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border-2 border-dashed border-blue-300 p-8 text-center hover:border-blue-500 hover:bg-blue-50 transition-colors cursor-pointer">
        <div className="flex flex-col items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-gray-900">Subir boleta o factura</p>
      ...
      {drafts.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-gray-900 mb-3">⚡ Borradores pendientes de revisión</h3>
          <div className="space-y-3">
            {drafts.map(gasto => (
              <div key={gasto.id} className="bg-white rounded-xl border border-yellow-200 p-5">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-gray-400">Subido: {gasto.fechaSubida}</span>
                      <span className="text-xs text-gray-400 font-mono">📎 {gasto.archivoNombre}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
                      <div className="bg-gray-50 p-3 rounded-lg">
                        ...
                        <p className="text-sm font-medium text-gray-900">{gasto.proveedorDetectado || "No detectado"}</p>
                      ...
                      <div className="bg-gray-50 p-3 rounded-lg">
                        ...
                        <p className="text-sm font-medium text-gray-900">{gasto.montoDetectado > 0 ? formatGuaranies(gasto.montoDetectado) : "No detectado"}</p>
                      ...
                      <div className="bg-gray-50 p-3 rounded-lg">
                        ...
                        <p className="text-sm font-medium text-gray-900">{gasto.fechaDocumento || "No detectada"}</p>
                      ...
                      <div className="bg-gray-50 p-3 rounded-lg">
                        ...
                        <p className="text-sm font-medium text-gray-900">{gasto.centroCostoSugerido ? getCentroCostoNombre(gasto.centroCostoSugerido) : "Sin clasificar"}</p>
      ...
      {procesando.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-gray-900 mb-3">🔄 Procesando OCR...</h3>
          <div className="space-y-2">
            {procesando.map(gasto => (
              <div key={gasto.id} className="bg-white rounded-xl border border-blue-200 p-4 flex items-center gap-4">
                <div className="w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                <div>
                  <p className="text-sm font-medium text-gray-900">{gasto.archivoNombre}</p>
      ...
      <div>
        <h3 className="text-sm font-medium text-gray-900 mb-3">✓ Gastos ratificados — Julio 2026</h3>
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Centro de costo</th>
                <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase">Monto</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Fecha doc.</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Acciones</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={gasto.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-600 font-mono">{gasto.archivoNombre}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">{gasto.proveedorFinal}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{gasto.centroCostoFinal ? getCentroCostoNombre(gasto.centroCostoFinal) : "—"}</td>
                  <td className="px-4 py-3 text-sm text-right font-medium text-gray-900">{formatGuaranies(gasto.montoFinal || 0)}</td>
                  <td className="px-4 py-3 text-sm text-gray-500">{gasto.fechaDocumento}</td>
                  <td className="px-4 py-3">
                    <button className="text-xs text-blue-600 hover:underline mr-2">Editar</button>
                    <button className="text-xs text-gray-500 hover:underline mr-2">Ver doc</button>
...
function GastosPresupuesto() {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-gray-900">Presupuesto de Gastos Comunes</h3>
        <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Agregar categoría</button>
      </div>
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase">Presupuesto mensual</th>
              <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase">% del total</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {categoriasGasto.map(cat => (
              <tr key={cat.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm text-right font-medium text-gray-900">{formatGuaranies(cat.presupuestoMensual)}</td>
...
function GastosCentrosCosto() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-gray-900">Centros de Costo</h3>
        <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Nuevo centro</button>
      </div>

      <p className="text-sm text-gray-600">Estructura jerárquica para clasificar y agrupar gastos. Se usa para el reconocimiento automático de boletas y para reportes.</p>

      <div className="space-y-3">
        {padres.map(padre => {
          const hijos = centrosCosto.filter(cc => cc.padre === padre.id);
          return (
            <div key={padre.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              {/* Padre */}
              <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-200">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-gray-900">{padre.nombre}</span>
                ...
                <div className="divide-y divide-gray-100">
                  {hijos.map(hijo => (
                    <div key={hijo.id} className="flex items-center justify-between px-5 py-2 pl-10">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-sm text-gray-500">{formatGuaranies(hijo.presupuestoMensual)}</span>
                        <button className="text-xs text-blue-600 hover:underline">Editar</button>
                        <button className="text-xs text-red-600 hover:underline">Eliminar</button>
                      </div>
                    </div>
      ...
      <div className="bg-gray-50 rounded-xl border border-gray-200 p-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div>
            ...
            <p className="text-lg font-bold text-gray-900">{centrosCosto.length}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500">Categorías padre</p>
            <p className="text-lg font-bold text-gray-900">{padres.length}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500">Presupuesto total</p>
            <p className="text-lg font-bold text-gray-900">{formatGuaranies(padres.reduce((s, p) => s + p.presupuestoMensual, 0))}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500">Prorrateables</p>
            <p className="text-lg font-bold text-gray-900">{centrosCosto.filter(c => c.prorrateable && !c.padre).length}/{padres.length}</p>
          </div>
        </div>
      </div>
    </div>
...
function BancoView() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Movimientos Bancarios — Julio 2026</h2>
        <div className="flex gap-2">
          <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Conciliar</button>
          <button className="px-4 py-2 border border-gray-300 text-sm rounded-lg hover:bg-gray-50">Importar extracto</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200">
          <p className="text-sm text-gray-500">Saldo del período</p>
      ...
      <div className="bg-gray-100 rounded-xl p-4 flex items-center justify-between">
        <div>
          <p className="text-sm text-gray-500">Cuenta: Banco Itaú Paraguay</p>
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase">Monto</th>
              <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase">Conciliado</th>
            </tr>
          </thead>
          <tbody>
            {movimientosBancarios.map(mov => (
              <tr key={mov.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-xs text-gray-400 font-mono">{mov.referencia}</td>
...
function ReclamosView() {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Reclamos y Solicitudes</h2>
        <div className="flex gap-2">
          <span className="text-sm text-gray-500">Total: {reclamos.length} | Abiertos: {reclamos.filter(r => r.estado !== "resuelto").length}</span>
        </div>
      </div>
      <div className="space-y-3">
        {reclamos.map(r => (
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-sm font-medium text-gray-900">{r.titulo}</h3>
                ...
                <div className="flex gap-4 mt-2">
                  <span className="text-xs text-gray-400">Depto {unidades.find(u => u.id === r.unidadId)?.numero} — {r.residente}</span>
                  <span className="text-xs text-gray-400">Categoría: {r.categoria}</span>
                  <span className="text-xs text-gray-400">Creado: {r.fechaCreacion}</span>
                </div>
              </div>
              <div className="flex gap-2">
...
function MantencionesView() {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Calendario de Mantenciones</h2>
        <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Programar mantención</button>
      </div>
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Proveedor</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Frecuencia</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Última</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Próxima</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Estado</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {mantenciones.map(m => (
              <tr key={m.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-4 py-3 text-sm font-medium text-gray-900">{m.equipo}</td>
                <td className="px-4 py-3 text-sm text-gray-700">{m.proveedor}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{m.frecuencia}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{m.ultimaMantencion}</td>
                <td className="px-4 py-3 text-sm text-gray-500">{m.proximaMantencion}</td>
                <td className="px-4 py-3">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Agregar proveedor</button>
      </div>
...
function ReportesView() {
  return (
    <div className="space-y-6">
      ...
      {/* Recaudación mensual */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-medium text-gray-900 mb-4">Recaudación vs Emitido (2026)</h3>
        <div className="space-y-2">
      ...
      {/* Morosidad */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-medium text-gray-900 mb-4">Evolución de Morosidad (%)</h3>
        <div className="space-y-2">
      ...
      {/* Pagos por canal */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-medium text-gray-900 mb-4">Distribución por Canal de Pago (Julio 2026)</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {pagosPorCanal.map(p => (
            <div key={p.canal} className="text-center p-3 bg-gray-50 rounded-lg">
              <p className="text-lg font-bold text-gray-900">{p.cantidad}</p>
              <p className="text-xs text-gray-500">transacciones</p>
              <p className="text-sm font-medium text-blue-600 mt-1">{p.canal}</p>
              <p className="text-xs text-gray-400">{formatGuaranies(p.monto)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button className="px-4 py-2 border border-gray-300 text-sm rounded-lg hover:bg-gray-50">📧 Enviar reporte por email</button>
      </div>
    </div>
  );
}
...
function LiquidacionView() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Emitir Liquidación Mensual</h2>
        <div className="flex gap-2 items-center">
          <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm">
            <option>Julio 2026</option>
            <option>Agosto 2026</option>
          </select>
          <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Generar liquidación</button>
        </div>
      </div>

      {/* Paso 1: Resumen de gastos por centro de costo */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-medium text-gray-900 mb-3">Paso 1: Gastos ratificados del período</h3>
        <div className="space-y-2">
          <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Gastos Básicos (ANDE + ESSAP)</span><span className="text-sm font-medium">₲ 4.800.000</span></div>
          <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Remuneraciones</span><span className="text-sm font-medium">₲ 8.050.000</span></div>
          <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Servicios contratados (seguridad, limpieza, jardín)</span><span className="text-sm font-medium">₲ 14.500.000</span></div>
          <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Mantención y reparaciones</span><span className="text-sm font-medium">₲ 2.500.000</span></div>
          <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Seguros</span><span className="text-sm font-medium">₲ 1.500.000</span></div>
          <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Administración</span><span className="text-sm font-medium">₲ 2.000.000</span></div>
          <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Fondo de reserva</span><span className="text-sm font-medium">₲ 3.000.000</span></div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-medium text-gray-900 mb-3">Paso 2: Prorrateo por unidad (coeficiente)</h3>
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase">Unidad</th>
                <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase">Residente</th>
                <th className="text-right px-4 py-2 text-xs font-medium text-gray-500 uppercase">Coef.</th>
                <th className="text-right px-4 py-2 text-xs font-medium text-gray-500 uppercase">% prorrateo</th>
                <th className="text-right px-4 py-2 text-xs font-medium text-gray-500 uppercase">Monto</th>
                <th className="text-center px-4 py-2 text-xs font-medium text-gray-500 uppercase">Ajuste</th>
              </tr>
            </thead>
            <tbody>
              {unidadesEdificio.slice(0, 8).map(u => {
                ...
                return (
                  <tr key={u.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-sm text-gray-600">{u.residente}</td>
                    ...
                    <td className="px-4 py-2 text-sm text-right font-medium text-gray-900">{formatGuaranies(monto)}</td>
      ...
      <div className="bg-green-50 border border-green-200 rounded-xl p-5">
        ...
        <div className="flex gap-3">
          ...
          <button className="px-4 py-2 border border-gray-300 text-sm rounded-lg hover:bg-gray-50">Guardar borrador</button>
          <button className="px-4 py-2 border border-gray-300 text-sm rounded-lg hover:bg-gray-50">Vista previa PDF</button>
        </div>
      </div>
    </div>
  );
...
function AsambleasView() {
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Asambleas</h2>
        <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Convocar asamblea</button>
      </div>
        ...
        <div key={a.id} className="bg-blue-50 border border-blue-200 rounded-xl p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="text-sm font-medium text-gray-900">{a.titulo}</h3>
      ...
      {/* Historial */}
      <div>
        <h3 className="text-sm font-medium text-gray-900 mb-3">Historial de asambleas</h3>
        <div className="space-y-3">
          {asambleas.filter(a => a.estado === "finalizada").map(a => (
            <div key={a.id} className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="text-sm font-medium text-gray-900">{a.titulo}</h4>
...
function VotacionesView() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Votaciones</h2>
        <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Crear votación</button>
      </div>

      {/* Votaciones activas */}
      {activas.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-gray-900 mb-3">🗳️ Votaciones activas</h3>
          <div className="space-y-4">
            {activas.map(v => (
              <div key={v.id} className="bg-white rounded-xl border border-green-200 p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-sm font-medium text-gray-900">{v.titulo}</h4>
      ...
      {/* Votaciones cerradas */}
      <div>
        <h3 className="text-sm font-medium text-gray-900 mb-3">Historial de votaciones</h3>
        <div className="space-y-3">
          {cerradas.map(v => (
            <div key={v.id} className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">Período: {v.fechaInicio} — {v.fechaCierre}</p>
                </div>
...
function AuditoriaView() {
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-gray-900">Log de Auditoría</h2>
        <div className="flex gap-2">
          <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm">
          ...
          <button className="px-4 py-2 border border-gray-300 text-sm rounded-lg hover:bg-gray-50">Exportar</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Fecha/Hora</th>
              ...
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Módulo</th>
          ...
          <tbody>
            {logsAuditoria.map(log => (
              <tr key={log.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm font-medium text-gray-900">{log.accion}</td>
                <td className="px-4 py-3 text-xs text-gray-500">{log.modulo}</td>
                <td className="px-4 py-3 text-xs text-gray-600 max-w-xs truncate">{log.detalle}</td>
...
function ConfigView() {
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-gray-900">Configuración — Torre Ñandutí</h2>
      
      {/* Datos del edificio */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-medium text-gray-900 mb-4">Datos del edificio</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-gray-500">Nombre</label>
            <input type="text" defaultValue="Torre Ñandutí" className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
          <div>
            <label className="text-xs text-gray-500">RUC</label>
            <input type="text" defaultValue="80012345-6" className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
          <div>
            <label className="text-xs text-gray-500">Dirección</label>
            <input type="text" defaultValue="Av. España 1245, Villa Morra" className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
          <div>
            <label className="text-xs text-gray-500">Ciudad</label>
            <input type="text" defaultValue="Asunción" className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-medium text-gray-900 mb-4">Configuración de cobro</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
          ...
          <div>
            ...
            <input type="number" defaultValue="5" className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
          <div>
            <label className="text-xs text-gray-500">Tasa de interés por mora (% mensual)</label>
            <input type="number" defaultValue="2" step="0.1" className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
          <div>
      ...
      {/* Canales de pago */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-medium text-gray-900 mb-4">Canales de pago habilitados</h3>
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
            <div className="flex items-center gap-3">
              <span className="text-lg">🏦</span>
              <div>
                <p className="text-sm font-medium text-gray-900">SPI — Transferencia Instantánea</p>
          ...
          <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
            <div className="flex items-center gap-3">
              <span className="text-lg">📱</span>
              <div>
                <p className="text-sm font-medium text-gray-900">Tigo Money</p>
          ...
          <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
            <div className="flex items-center gap-3">
              <span className="text-lg">📲</span>
              <div>
                <p className="text-sm font-medium text-gray-900">Personal Pay</p>
      ...
      <button className="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Guardar cambios</button>
    </div>
  );
}
